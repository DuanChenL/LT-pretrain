...
class TwoStageDetector(BaseDetector):
    ...
    def forward_train(self,
        ...
        for i in range(len(select_proposal)):
            ...
            scale = torch.tensor(img_metas[i]['scale_factor']).to(device)
            selective_search_proposal['view0'][i] = selective_search_proposal['view0'][i] * scale
            if img_metas[i]['flip']:
                selective_search_proposal['view0'][i] = bbox_flip(selective_search_proposal['view0'][i], img_metas[i]['img_shape'], img_metas[i]['flip_direction'])
            ...
            if img_metas[i]['flip']:
                selective_search_proposal['view1'][i] = bbox_flip(selective_search_proposal['view1'][i], img_metas[i]['img_shape'], img_metas[i]['flip_direction'])
        ...
        roi_losses = self.roi_head.forward_train([x, view1, view2], img_metas, proposal_list,
                                                 gt_bboxes, gt_labels,
                                                 gt_bboxes_ignore, gt_masks, selective_search_proposal,
                                                 **kwargs)
