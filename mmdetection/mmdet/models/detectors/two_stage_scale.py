...
class TwoStageDetector(BaseDetector):
    ...
    def forward_train(self,
        ...
        img_downsample = F.interpolate(img_2, scale_factor=0.5)
        x = self.extract_feat(img)
        ...
        with torch.no_grad():
            ...
            x_sim_down = self.extract_sim_feat(img_downsample)
            x_sim_g = torch.Tensor(x_sim[-1].mean(-1).mean(-1))
            k = x_sim_g
            k = self.proj_k(k)
            k = torch.nn.functional.normalize(k, dim=1)
            x_sim_g_down = torch.Tensor(x_sim_down[-1].mean(-1).mean(-1))
            k_down = x_sim_g_down
            k_down = self.proj_k(k_down)
            k_down = torch.nn.functional.normalize(k_down, dim=1)
        ...
        l_pos_down = torch.einsum("nc,nc->n", [q, k_down]).unsqueeze(-1)
        ...
        self._dequeue_and_enqueue(k_down)
        losses = dict()
        ...
        if labels_ssl.shape[0] == logits.shape[0]:
            loss_ssl_ = self.loss_ssl(
                logits,
                labels_ssl)
            loss_ssl_down = self.loss_ssl(
                logits_down,
                labels_ssl)
            loss_ssl_ = 0.1 * (loss_ssl_ + loss_ssl_down) / 2
