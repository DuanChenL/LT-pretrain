...
class MultiViewDataset:
    def __init__(self, dataset, oversample_thr, num_views, pipelines, filter_empty_gt=True):
        ...
        for dataset_idx, repeat_factor in enumerate(repeat_factors):
            num_samples = int(repeat_factor)  # 整数部分确定采样次数
            decimal_part = repeat_factor - num_samples  # 小数部分确定额外的采样次数
            if random.random() < decimal_part:  # 根据小数部分的概率确定是否额外采样一次
                num_samples += 1
            self.repeat_factors.append(num_samples)
            # repeat_indices.extend([dataset_idx] * math.ceil(repeat_factor))
            repeat_indices.extend([dataset_idx] * num_samples)
        self.repeat_indices = repeat_indices

        flags = []
        if hasattr(self.dataset, 'flag'):
            for flag, repeat_factor in zip(self.dataset.flag, self.repeat_factors):
                # flags.extend([flag] * int(math.ceil(repeat_factor)))
                flags.extend([flag] * repeat_factor)
            assert len(flags) == len(repeat_indices)
        self.flag = np.asarray(flags, dtype=np.uint8)
        ...
        for pipe in pipelines:
            if isinstance(pipe, tuple):
                pipeline = []
                for pip in pipe:
                    pipeline_ = Compose([build_from_cfg(p, PIPELINES) for p in pip])
                    pipeline.append(pipeline_)
            else:
                pipeline = Compose([build_from_cfg(p, PIPELINES) for p in pipe])
            self.pipelines.append(pipeline)

    def _get_repeat_factors(self, dataset, repeat_thr, t=0):
        """Get repeat factor for each images in the dataset.
        ...
        category_freq = defaultdict(int)
        category_bbox_freq = defaultdict(int)
        category_freq_ = defaultdict(int)
        bbox_nums = 0
        num_images = len(dataset)
        for idx in range(num_images):
            cat_ids = set(self.dataset.get_cat_ids(idx))
        ...
        for k, v in category_freq.items():
            # category_freq[k] = math.sqrt((v / num_images) * category_bbox_freq[k])
            category_freq[k] = ((v / num_images) * category_bbox_freq[k]) / ((1 - t) * (v / num_images) + t * category_bbox_freq[k])
        ...
        category_repeat_ = {
            cat_id: max(0.0, math.sqrt(repeat_thr / cat_freq))
            for cat_id, cat_freq in category_freq.items()
        }
    ...
    def __getitem__(self, idx):
        ...
        #         result = pipeline[0](result)
        #         x = deepcopy(result)
        #         results_1 = pipeline[2](result)
        #         results_2 = pipeline[1](x)
        #         results_2 = pipeline[2](results_2)
        #         res.append(results_1)
        #         res.append(results_2)
        #         # results_1['sim'] = results_2
        #     else:
        #         res.append(pipeline(deepcopy(results)))
        # return res
    ...
    def dynamic_update(self, t):
        repeat_factors = self._get_repeat_factors(self.dataset, self.oversample_thr, t)
        for i, repeat_factor in enumerate(self.repeat_factors):
            if repeat_factor == 0:
                repeat_factors[i] = max(1.0, repeat_factors[i])
        repeat_indices = []
        for dataset_idx, repeat_factor in enumerate(repeat_factors):
            num_samples = int(repeat_factor)  # 整数部分确定采样次数
            decimal_part = repeat_factor - num_samples  # 小数部分确定额外的采样次数
            if random.random() < decimal_part:  # 根据小数部分的概率确定是否额外采样一次
                num_samples += 1
            self.repeat_factors[dataset_idx] = num_samples
            repeat_indices.extend([dataset_idx] * num_samples)
        self.repeat_indices = repeat_indices
        flags = []
        if hasattr(self.dataset, 'flag'):
